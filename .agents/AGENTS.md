...
- **Frontend & UI/UX Rule Synchronization (Tuân thủ toàn diện RuleFE.md)**:
  ...
  - **Bảo toàn Tuyệt đối Luồng 2 Tính năng Key khi Chỉnh sửa UI (Immutable 2 Key ML Feature Flows Rule)**: Dù tinh chỉnh, thay đổi bố cục hoặc thiết kế lại UI/UX (Frontend) ở bất kỳ màn hình nào, TUYỆT ĐỐI BẢO ĐẢM GIỮ NGUYÊN VẸN LUỒNG HOẠT ĐỘNG (User Flow & Data Flow) của 2 tính năng key: (1) Đăng ký sinh viên 24 góc quét camera & huấn luyện gallery ArcFace; (2) Camera điểm danh thời gian thực nhận diện khuôn mặt trên Dashboard. Mọi can thiệp giao diện chỉ được tinh chỉnh thẩm mỹ, tuyệt đối không được làm gián đoạn hay phá vỡ luồng xử lý của 2 tính năng này.
  - Khi cập nhật rule xong, hỏi lại người dùng bằng tiếng Việt và không tự ý sửa đổi mã nguồn.
